...
See docs/reference/laz_roundtrip_design.md.
...
docs/reference/voxelizer-documentation.md section 13.
...
Viewing the export: `python -m voxelizer serve` serves a run directory with
the CORS and no-cache headers browsers need and opens the bundled CesiumJS
viewer page beside it -

    python -m voxelizer serve outputs/Run1     # --open-viewer itowns for iTowns

No Cesium ion account is needed. (An Unreal Engine path existed in the
private workspace; it is not part of this delivery.)
...
./entire-lyon-outputs bind mounts).
...
`view_itowns.cmd`. Launchers carry no absolute
...
CAVEATS:
  ...
  * For metropolis-scale passes, run in blocks under process isolation so a
    native fault costs one block rather than the run (see how-to-use.md).
