...
with `ProblemSolving.md` (every problem hit and how it was solved)
and `BIBLIOGRAPHY_AND_REFERENCES.md` (the source of record for every
...
- **Why a parent's bounding volume is the UNION of its children's, not its slot in
  ...
  discard a visible descendant. Full story in `ProblemSolving.md` 5.7.
...
- **Why `geometricError` is 8 x voxel size rather than the voxel size itself**: the
  ...
  makes the client threshold the tuning knob rather than the export (measured sweep:
  `tilesexport/AssistingRuns/docs/01_pipeline_overview.md`). Cost: the published
