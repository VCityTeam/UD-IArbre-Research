"""
Download Grand Lyon 2023 orthophoto (.tif) tiles for an RGF93/CC46 (EPSG:3946) bounding box.
@ingroup t0_socle


Reads the inventory JSON and streams every tile whose origin falls inside
the requested rectangle into an output directory (default ``inputs/ortho/``).

Unlike the vegetalisation workflow, ortho tiles are downloaded as-is (no
resampling).  Use ``ortho_extract.py`` from the vegetalisation codebase if
you need source-to-target resampling.

Why this is its own module rather than a switch on one downloader: what
``_download_common`` shares with ``download_laz`` is the per-file fetch
...
``download_laz`` (57 lines each, 53 of them byte-identical, longest
identical run 35 lines). Only the per-dataset surface differs - the Grand
Lyon 2023 orthophotography inventory, the ``inputs/ortho/`` destination,
the ``[ortho]`` log tag and this module's own ``-m`` entry point - so each
command stays usable on its own. The twin is kept in pairs: change one and
check the other.

Examples
--------
    # Preview which tiles the default box selects
    python -m voxelizer.download_orthos --dry-run
    ...
    python -m voxelizer.download_orthos --limit 5 ...

    # NOTE: must be run with ``-m`` - the module uses package-relative
    # imports, so ``python voxelizer/download_orthos.py`` raises ImportError
"""
...
DEFAULT_JSON = resolve_inventory(
    "dalles-d-orthophotographie-2023-de-la-metropole-de-lyon.json")
DEFAULT_ORTHO_DIR = _INPUTS_DIR / "ortho"
...
def download_orthos(
    ...
    *,
    ...
    """Download orthophoto tiles inside *bbox*. Returns the number of failures."""
    if workers is None:
    ...
    print(f"[ortho] {len(selected)} tile(s) selected"
    ...
    print(f"[ortho] done: {downloaded} downloaded, {skipped} already present, "
...
def _parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    """Build the parser (bounding-box ints, ``--json``, ``--ortho-dir``, ``--workers``, ``--limit``, ``--dry-run``) and parse *argv*."""
    p = argparse.ArgumentParser(
        prog="download_orthos",
        description="Download Grand Lyon 2023 orthophoto tiles for a "
    ...
    p.add_argument("--json", type=Path, default=DEFAULT_JSON,
                   help="Orthophoto inventory JSON.")
    p.add_argument("--ortho-dir", type=Path, default=DEFAULT_ORTHO_DIR,
                   help="Destination for .tif tiles.")
...
def main(argv: list[str] | None = None) -> None:
    """Command-line entry point: parse the bounding-box and download flags, print the box and run download_orthos().
    ...
    failures = download_orthos(
        bbox, args.ortho_dir, args.json,
