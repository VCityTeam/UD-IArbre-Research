...
beside a `*_stream.html`, and `view_cesium.cmd` / `view_itowns.cmd` /
`launch_unreal.cmd` beside a `tileset.json` - so a specific result opens with
...
All commands below assume you are in the `VoxelisingPython\` directory and have
...
Both converters also write the `view_cesium.cmd`, `view_itowns.cmd` and
`launch_unreal.cmd` launchers beside `tileset.json`.
...
Unreal path. For the three server launchers the console window that opens is
the server's lifetime, and closing it stops the server; each file says so in
its own header. `launch_unreal.cmd` starts no server: it hands the project to
the editor and exits.
