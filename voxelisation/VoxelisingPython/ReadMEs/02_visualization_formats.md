...
- The shipped guards are the module-level `assert _REC_BYTES == 32` in both
  ...
  `ReadMEs/gen_export_paths.py` also reads these values out of the source
  and asserts them before drawing, but it is a maintainer tool kept out of
  publication by `VoxelisingPython/.gitignore` (with `gen_module_tiers.py`
...
and the Docker GUI service, it is documented in `HowToUse.md` and the report,
