# ProblemSolving.md - every problem this pipeline hit, and how it was solved
...
`DESIGN_DECISIONS.md` (why each design choice was made, with alternatives
and costs) and `BIBLIOGRAPHY_AND_REFERENCES.md` (every source, with
per-section traceability). The report argues the same material at design
level; this file is the engineering record behind it.
`tilesexport/AssistingRuns/docs/03_troubleshooting.md` covers the 3D Tiles
specifics.
...
- **Issue.** LiDAR returns on vertical surfaces (facades, trunks) create
  ...
  (`DESIGN_DECISIONS.md` section 1) - that 2x is the defensible figure.
...
- **Solution.** `root.transform` built via pyproj (East-North-Up frame at
  ...
  Terrain); the exporter now applies this correction automatically (see
  `tilesexport/AssistingRuns/docs/03_troubleshooting.md` and
  `tilesexport/UEViz/PIPELINE.md`), with a manual per-viewer height nudge
...
- **Solution.** Four rules, in `UEViz/PIPELINE.md`: physics meshes off
...
- **Alternatives.** Smaller region tilesets for physics work (E7 in
  `UEViz/DETAIL_EXPERIMENTS_PLAN.md`); greedy meshing (roadmap).
...
- **Side effects.** Measured on the exported
  `tilesexport/Run1/tileset.json` (the run tree itself is a 6.1 GB
...
- **Repaired in place.** Only `tileset.json` carries boxes, so the
  shipped artefacts needed no GLB rewrite:
  `tilesexport/AssistingRuns/patch_tileset_bv.py` re-applies the same
  union to an already-exported tileset (axis-aligned boxes only behind
  a hard assert, `--audit-only` to measure without writing,
  `tileset.json.bak-prebvfix` backups). Both `tilesexport/Run1` and its iTowns-adapted copy (a working copy outside this delivery) now audit 0 violations
  and 0 root escapees.
...
- **What the pin rests on TODAY.** The memory corruption itself, and
  ...
  (`DESIGN_DECISIONS.md` section 11). Any external advice to "switch to
...
choice-by-choice rationale is `DESIGN_DECISIONS.md`; the sources are
