...
def main():
    ...
    for time_index in range(time_index, time_index_end) : # ligne a retirer
        print("loop")
        ...
        shadow_combined = np.unique(  # ligne a modifier : shadow_combined -> shadow_all
            np.concatenate((shadow, shadow_vegetation), axis=0),
            axis=0
        )
        shadow_all.append(shadow_combined)   # ligne a retirer
# DEBUG
        print("type shadow_all :", type(shadow_all))
        print("len shadow_all :", len(shadow_all))
        #print("shape shadow_all :", np.asarray(shadow_all).shape)
        #print("premier élément :", shadow_all[0])
        #print("type premier élément :", type(shadow_all[0]))
# DEBUG

    #shadow_all = np.unique(  # ligne a retirer
    #    np.concatenate(shadow_all, axis=0), # ligne a retirer
    #    axis=0 # ligne a retirer
    #) # ligne a retirer
    multiple_hours_shadow_save_tiff_format(N, shadow_all, int(x_min), int(y_min), DSM, 1.0, "Method2_georef") #ligne a retirer

# too much data a remettre
