...
def main():
    ...
    if time_index_end != 99 :
        for time_index in range(time_index, time_index_end+1) : # ligne a retirer
            ...
            shadow_all = np.unique(  # ligne a modifier : shadow_combined -> shadow_all
                np.concatenate((shadow, shadow_vegetation), axis=0),
                axis=0
            )
            shadow_all_period.append(shadow_all)   # ligne a retirer
        multiple_hours_shadow_save_tiff_format(N+1, shadow_all_period, int(x_min), int(y_min), DSM, 1.0, "Method2_multiple_hours_shadow")  # ligne a retirer
...
# data
    ...
    georef_shadow_save(shadow_all, int(x_min), int(y_min), DSM, 1.0, f"Method2_georef_{time_index_end}") #add hours
