...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--laz-folder", type=Path, default=Path("./workdir/runs/test_batiments/lidar/Confluences/mosaic"))
    parser.add_argument(
        "--height-folder",
        type=Path,
        default=Path("./workdir/runs/test_batiments/lidar/Confluences/heights"),
    )
    parser.add_argument(
        "--class-folder",
        type=Path,
        default=Path("./workdir/runs/test_batiments/lidar/Confluences/class"),
    )
    parser.add_argument(
        "--mns-mnt-folder",
        type=Path,
        default=Path("./workdir/runs/test_batiments/lidar/Confluences/mns_mnt"),
    )
...
def clean_simple(input_array: np.ndarray, input_class: np.ndarray) -> np.ndarray:
...
def clean_mnt_mns(input_array: np.ndarray, input_class: np.ndarray) -> np.ndarray:
    ...
    # Identifier les points connus et les points à interpoler
    points_connus = np.where(~np.isnan(input_array))
    valeurs_connues = input_array[points_connus]
    points_vides = np.where(np.isnan(input_array))

    # griddata prend les coordonnées (X, Y)
    coords_connues = np.array(list(zip(points_connus[0], points_connus[1])))
    coords_vides = np.array(list(zip(points_vides[0], points_vides[1])))

    # Interpolation linéaire
    valeurs_interpolees = griddata(coords_connues, valeurs_connues, coords_vides, method='linear')
    input_array[points_vides] = valeurs_interpolees

    # Clean simple au cas où il resterait des NaN après l'interpolation
    input_array = clean_simple(input_array, input_class)

    # 3. Lissage 
    mnt_lisse = gaussian_filter(input_array, sigma=1.0) # sigma=1 correspond environ à un noyau 3x3 ou 5x5
...
def create_mns_mnt_class(
    ...
    # On parcourt les points pour remplir les rasters MNS, MNT et de classes
    # On exclut les végétations et les bâtiments pour le MNT
    ...
    mnt = clean_mnt_mns(mnt, class_raster)

    write_raster(out_mns, mns, crs, transform, nodata=np.nan)
    write_raster(out_mnt, mnt, crs, transform, nodata=np.nan) # clean_mnt_mns(mnt, class_raster)
