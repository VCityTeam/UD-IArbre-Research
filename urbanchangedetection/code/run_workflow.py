...
DEFAULT_CHANNELS_BY_BAND_MODE = {
    "rgb": [1, 2, 3],
    # Le raster IR téléchargé (dossier "infrarouge" Grand Lyon) est déjà un composite
    # 3 bandes dans l'ordre attendu par le modèle FLAIR-HUB IR (pas besoin de réordonner
    # ni de supposer une 4ème bande) : les canaux physiques [1, 2, 3] du fichier
    # suffisent, exactement comme pour le RGB.
    "ir": [1, 2, 3],
}
...
# Valeurs par défaut du modèle FLAIR-HUB selon --band-mode. La révision RGB est
# pinnée sur un commit précis (reproductibilité). Pour l'IR, on ne dispose pas
# encore d'un hash de commit fiable : "main" pointe vers la dernière version du
# modèle. Si tu veux figer une version précise, va sur la page HuggingFace du
# modèle IR ("Files and versions" -> historique des commits) et renseigne le hash
# ici, ou passe --model-revision explicitement.
DEFAULT_MODEL_CONFIG_BY_BAND_MODE = {
    "rgb": {
    ...
    "ir": {
        "repo": "IGNF/FLAIR-HUB_LC-A_IR_swinlarge-upernet",
        "filename": "FLAIR-HUB_LC-A_IR_swinlarge-upernet.safetensors",
        "revision": "main",
    },
}
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "--channels et le choix du modèle FLAIR-HUB (voir --model-repo)."
        ),
    )
    parser.add_argument(
        "--channels",
    ...
    parser.add_argument(
        ...
        "Si omis, calculées automatiquement sur la mosaïque orthophoto du run.",
    )
    parser.add_argument(
        "--stds",
        type=float,
        ...
        "Si omis, calculés automatiquement sur la mosaïque orthophoto du run.",
    )
...
def resolve_model_reference(args: argparse.Namespace) -> None:
    """Complète args.model_repo/model_filename/model_revision avec les valeurs par
    défaut du --band-mode courant, pour chaque champ laissé à None par l'utilisateur.
    Ne touche jamais à --model-path (chemin local explicite, toujours prioritaire).
    """
    defaults = DEFAULT_MODEL_CONFIG_BY_BAND_MODE[args.band_mode]
    if args.model_repo is None:
        args.model_repo = defaults["repo"]
    if args.model_filename is None:
        args.model_filename = defaults["filename"]
    if args.model_revision is None:
        args.model_revision = defaults["revision"]
...
def write_runtime_config(
    ...
    *,
    ...
    if not (len(channels) == len(means) == len(stds)):
        raise ValueError(
            f"channels ({len(channels)}), means ({len(means)}) et stds ({len(stds)}) "
    ...
    config["modalities"]["AERIAL_RGBI"]["channels"] = channels
    config["modalities"]["AERIAL_RGBI"]["normalization"]["means"] = means
    config["modalities"]["AERIAL_RGBI"]["normalization"]["stds"] = stds
    ...
    *,
    ...
    settings = {
        ...
        "ortho_output_resolution": float(
            args.ortho_output_resolution
            if args.ortho_output_resolution is not None
            else (
                args.resolution
                if args.resolution is not None
                else orthophoto_config.get("output_resolution", 0.2)
            )
        ),
    ...
    # --- Bandes orthophoto (RGB/IR) ---
    # Le fallback dépend de --band-mode : RGB et IR utilisent tous deux [1,2,3] par
    # défaut, car dans les deux cas le raster source a déjà ses bandes dans l'ordre
    # attendu par le modèle FLAIR-HUB correspondant. --channels reste disponible pour
    # les cas particuliers (ex: mosaïque à bandes réordonnées ou supplémentaires).
    default_channels = DEFAULT_CHANNELS_BY_BAND_MODE[args.band_mode]
    settings["band_mode"] = args.band_mode
    settings["channels"] = [
        int(c) for c in choose(args.channels, orthophoto_config.get("channels"), default_channels)
    ]

    # means/stds n'ont volontairement PAS de fallback ici : s'ils ne sont fournis ni
    # en CLI ni dans le matrix_config, on les calcule automatiquement plus tard sur
    # la mosaïque orthophoto réelle du run (voir compute_band_stats), car des valeurs
    # par défaut génériques n'ont pas de sens d'une résolution/emprise à l'autre.
    means_value = choose(args.means, orthophoto_config.get("means"), None)
    stds_value = choose(args.stds, orthophoto_config.get("stds"), None)
    settings["means"] = [float(m) for m in means_value] if means_value is not None else None
    settings["stds"] = [float(s) for s in stds_value] if stds_value is not None else None
...
# Fonctionnement du main workflow :
#
# === DONNÉES D'ENTRÉE ===
# 1. Extraction des tuiles LiDAR (.laz) depuis l'inventaire JSON (bbox xmin/ymin)
# 2. Extraction et rééchantillonnage des tuiles orthophotos (résolution cible : 0.5m par défaut)
# peut-être changer pour mettre 0.2m si on veut un meilleur résultat FLAIR, mais ça va être plus lourd
#
# === PRÉPARATION DES RASTERS LiDAR ===
# 3. Rasterisation du nuage de points LiDAR (fusion_nuage.py) :
#    produit pour chaque tuile : hauteur relative, classe LiDAR, MNS, MNT
# 4. Mosaïque des tuiles de hauteur LiDAR → lidar_height.tif
# 5. Mosaïque des tuiles MNS → lidar_mns.tif  (surface : bâtiments, végétation, sol...)
# 6. Mosaïque des tuiles MNT → lidar_mnt.tif  (terrain nu, sans objets)
#    Note : ortho_fusion.py est réutilisé ici pour fusionner les tuiles LiDAR,
#    car c'est une simple mosaïque raster indépendante du type de données.
# 7. Mosaïque des tuiles de classes LiDAR → lidar_class.tif
#
# === PRÉPARATION DES ORTHOPHOTOS ===
# 8. Mosaïque des tuiles orthophotos → orthophoto_mosaic.tif
#
# === INFÉRENCE FLAIR-HUB ===
# 9. Inférence FLAIR-HUB sur la mosaïque orthophoto :
#    produit un raster de probabilités par classe (19 bandes, une par classe)
#
# === POST-TRAITEMENT FLAIR ===
# 10. Repondération des probabilités FLAIR + argmax + filtrage (flair_probs_reweight.py) :
#     - Booste la probabilité de la classe bâtiment (classe 0) avant l'argmax
#     - Extrait uniquement la classe bâtiment → masque binaire FLAIR bâtiment
#
# === FUSION LiDAR + FLAIR ===
# 11. Fusion LiDAR/FLAIR (fusion_lidar_flair.py) :
#     - Prend le masque bâtiment FLAIR
#     - Invalide les pixels où le LiDAR indique du sol (hauteur < seuil ou classe LiDAR = sol)
#     - Produit building_map.tif : bâtiments FLAIR corrigés par le LiDAR
#
# === PIPELINE LEGACY (optionnelle, run_legacy_fusion=true) ===
# 12. Correction NaN sur le MNS (lidarCorrection.py) :
#     interpolation des pixels invalides dans le MNS avant calcul de hauteur
# 13. Calcul des classes LiDAR "heritage" (calculateVegetationFromLidar.py) :
#     ⚠️ Script orienté végétation, à adapter pour les bâtiments si la pipeline legacy est conservée
# 14. Fusion legacy LiDAR + FLAIR (fusionBetweenFlairAndLidar.py) :
#     variante plus ancienne de l'étape 11, sans la logique de correction par hauteur
#
# === ÉVALUATION (optionnelle) ===
# 15. Matrice de confusion (confusionMatrix.py) si un raster de référence est fourni
def main() -> None:
    args = parse_args()
    ...
    # Convention de dossiers: inputs/RGB/ortho.json ou inputs/IR/ortho.json selon --band-mode,
    # utilisée seulement si --ortho-json n'est pas fourni explicitement.
    ortho_json = (
        args.ortho_json or (inputs_dir / args.band_mode.upper() / "ortho.json")
    ).resolve()
    ...
    if not args.skip_download:
        ...
        # On commence par extraire le nuage LiDAR
        run_command(
            [
                sys.executable,
                "extract_nuage.py",
        ...
        # Ensuite, on extrait les tuiles orthophotos
    ...
    if args.reuse_derived_rasters:
        ...
        # On fusionne les hauteurs LiDAR
        run_command(
            [
                sys.executable,
                "ortho_fusion.py",
        ...
        if mns_count:
            ...
            # ⚠️ On fusionne les tuiles MNS (pk ?)
            run_command(
                [
                    sys.executable,
                    "ortho_fusion.py",
        ...
        if mnt_count:
            ...
            # ⚠️ On fusionne les tuiles MNT (pk ?)
            run_command(
                [
                    sys.executable,
                    "ortho_fusion.py",
        ...
        # ⚠️ On fusionne les tuiles de classes LiDAR
        # Mais pourtant on utilise ortho_fusion.py pour fusionner les tuiles de classes LiDAR, c'est bizarre.
        run_command(
            [
                sys.executable,
                "ortho_fusion.py",
        ...
        # On fusionne les tuiles orthophotos
        # Cette fois c'est logique.
        run_command(
            [
                sys.executable,
                "ortho_fusion.py",
    ...
    if not args.skip_flair:
        ...
        if means is None or stds is None:
            ...
            print(
                f"[auto] means/stds calculés sur {orthophoto_mosaic.name} "
        ...
        # On lance l'inférence FLAIR-HUB sur les orthophotos fusionnées
        run_command(
    ...
    if not args.skip_reweight:
        ...
        # On reweight les probas FLAIR-HUB
        reweight_command = [
            sys.executable,
    ...
    """
    # On fait la fusion entre les classes LiDAR et les probas FLAIR-HUB reweightées
    fusion_command = [
        sys.executable,
        ...
        str(lidar_height_mosaic),
        "--build-mask",
        str(reweighted_raster if reweighted_raster.exists() else probability_raster), # reweighted_raster
        "--out-dir",
        str(fusion_dir),
        "--matrix-config",
        str(matrix_config_path),
    ...
    if workflow_settings["run_legacy_fusion"]:
        ...
        if workflow_settings["apply_lidar_correction"]:
            ...
            # On applique la correction NaN sur le MNS avant de lancer le calcul de végétation LiDAR
            run_command(
                [
                    sys.executable,
                    "lidarCorrection.py",
        ...
        # On calcule la végétation LiDAR à partir du MNS corrigé et du MNT
        # ⚠️ Faire ça pour les bâtiments
        run_command(
            [
                sys.executable,
        ...
        # On fusionne les classes LiDAR héritées avec les probas FLAIR-HUB reweightées
        # Je vois pas trop la différence avec la fusion précédente. 
        run_command(
            [
                sys.executable,
    ...
    if args.reference_raster:
        ...
        # On génère la matrice de confusion, jsp si on le fera pour les bâtiments
        evaluation_command = [
            sys.executable,
            "confusionMatrix.py",
            "--reference",
